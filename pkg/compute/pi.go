	...
	return []PICalculationInfo{
		{
			...
			Description: "Chudnovsky algorithm - fastest convergence (~14.18 digits per iteration)",
			Speed:       "Fastest",
			Precision:   "Excellent",
			Complexity:  "High",
			Estimate:    EstimateCalculationTime(digits, string(MethodChudnovsky)),
		},
		{
			Method:      string(MethodMachin),
			Description: "Machin's formula - π/4 = 4*arctan(1/5) - arctan(1/239)",
			Speed:       "Moderate",
			Precision:   "Good",
			Complexity:  "Medium",
			Estimate:    EstimateCalculationTime(digits, string(MethodMachin)),
		},
		{
			Method:      string(MethodBailey),
			Description: "Bailey-Borwein-Plouffe formula - allows hexadecimal digit extraction",
			Speed:       "Good",
			Precision:   "Good",
			Complexity:  "Medium",
			Estimate:    EstimateCalculationTime(digits, string(MethodBailey)),
		},
	}
}
